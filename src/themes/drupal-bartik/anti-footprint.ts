...
const config: ThemeAntiFootprint = {
    ...
    sitemap: {
        // Drupal's Simple XML Sitemap module canonical XSL path.
        // The accompanying generator comment is the module's
        // standard giveaway — emitting it reinforces the
        // Drupal fingerprint at near-zero cost.
        xslHref: '/sitemap_generator/default/sitemap.xsl',
        xslBody: sitemapXsl,
        generatorComment: 'Generated by the Simple XML Sitemap Drupal module: https://drupal.org/project/simple_sitemap.',
    },

    // Drupal core's stock robots.txt with one deliberate omission:
    // the `Crawl-delay: 10` directive. Google ignores it, but
    // Bing / Yandex / Baidu respect it — keeping it would slow
    // initial indexing on those engines for no anti-footprint
    // gain that the rest of the file doesn't already provide
    // (the `/core/` + `/profiles/` listings, the dual clean-URL /
    // index.php blocks, the boilerplate comment header).
